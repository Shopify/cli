...
4. Go through all the [PRs labeled with `includes-post-release-steps`](https://github.com/Shopify/cli/issues?q=label%3Aincludes-post-release-steps+is%3Aclosed):
   1. Check if there are any post-release steps.
   2. If documentation changes are included:
      1. Follow those steps.
      1. Retain the labels so that we can remove them when we actually release a new version.
   3. If the steps do not include documentation changes:
      1. Follow those steps.
      2. Delete the labels.
