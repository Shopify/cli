    ...
    tasks = tasks.concat([
      ...
      },
      {
        ...
        task: async (_, task) => {
          ...
          git.createGitIgnore(templateScaffoldDir, git.GitIgnoreTemplate.Hydrogen)
          try {
            await git.commitAll('Initial commit generated by create-hydrogen', {directory: templateScaffoldDir})
            // eslint-disable-next-line no-catch-all/no-catch-all
          } catch (error) {
            // We couldn't commit in already initialized git repo, maybe the commit author config is not set.
            // In the future, we might supply our own committer like Ember CLI does, but for now, let's just
            // remove the Git files to avoid a half-done state.
            await cleanup(templateScaffoldDir)
            task.title = 'Failed to initialize repository'
            return
          }
