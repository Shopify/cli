import {selectOrg, selectOrganizationFromList} from './select.js'
...
describe('selectOrganizationFromList', () => {
  test('returns org matching flag ID from provided organizations', async () => {
    const result = await selectOrganizationFromList(ORGS, '5678')
  ...
  test('throws AbortError when flag ID does not match any provided organization', async () => {
    await expect(selectOrganizationFromList(ORGS, '9999')).rejects.toThrow('Organization with ID 9999 not found.')
  })

  test('falls back to prompt when no flag is provided', async () => {
    vi.mocked(selectOrganizationPrompt).mockResolvedValue(ORGS[0]!)

    const result = await selectOrganizationFromList(ORGS)

    expect(result).toEqual({id: '1234', businessName: 'My Org'})
    expect(selectOrganizationPrompt).toHaveBeenCalledWith(ORGS)
  })

  test('throws AbortError when no organizations are provided', async () => {
    await expect(selectOrganizationFromList([])).rejects.toThrow('No organizations found.')
  })
})
