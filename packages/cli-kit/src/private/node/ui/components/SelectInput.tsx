...
function SelectInput<T>({
  ...
  if (loading) {
        ...
        {noItems ? (
          ...
          <Box marginLeft={3} flexDirection="column">
            <Text dimColor>
              ...
              }to confirm.${descriptionsEnabled ? ' · ⇧⇥ full description' : ''}`}
            </Text>
            {hasMorePages ? (
              <Text>
    ...
    // Narrow terminals: show only a single, truncated preview line of the focused item's
    // description below the list (the focused row already shows its label). This costs exactly one
    // row (reserved via `listAvailableLines`), keeping the total height within the viewport. The
    // full text is one Shift+Tab away. The box stretches to `twoThirds`, so `marginLeft` leaves a
    // bound for `truncate-end` to clip against.
    return (
      <Box flexDirection="column" ref={ref} gap={1} width={twoThirds}>
        {listSection}
        <Box marginLeft={3}>
