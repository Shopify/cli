...
import {skipNetworkLevelRetry} from './environment.js'
...
vi.mock('./environment.js', async (importOriginal) => {
  ...
  const skipNetworkLevelRetryMock = vi.fn().mockReturnValue(false)
  return {
    ...actual,
    maxRequestTimeForNetworkCallsMs: () => DURATION_UNTIL_ABORT_IS_SEEN,
    skipNetworkLevelRetry: skipNetworkLevelRetryMock,
  }
})
...
describe('shopifyFetch', () => {
  ...
  test('abort signal is seen as a retryable error', async () => {
    ...
    // - the next delay would take us over our 500ms allowance, so retries are stopped
...
describe('downloadFile', () => {
  ...
  test('Cleans up if download fails', async () => {
    await inTemporaryDirectory(async (tmpDir) => {
      // Given
...
describe('requestMode', () => {
  test('no preference uses default', () => {
    ...
    expect(requestMode()).toEqual(expected)
    expect(requestMode('default')).toEqual(expected)
  })

  test('non-blocking requests', () => {
    expect(requestMode('non-blocking')).toEqual({
      useNetworkLevelRetry: false,
      useAbortSignal: true,
      timeoutMs: 100,
    })
  ...
  test('slow-request requests', () => {
    expect(requestMode('slow-request')).toEqual({
      useNetworkLevelRetry: false,
      useAbortSignal: false,
    })
  })
  ...
  test('default, with networkLevelRetry disabled', () => {
    vi.mocked(skipNetworkLevelRetry).mockReturnValue(true)
    expect(requestMode('default')).toMatchObject({
      useNetworkLevelRetry: false,
      useAbortSignal: true,
      timeoutMs: 100,
    })
    vi.mocked(skipNetworkLevelRetry).mockReturnValue(false)
  })
