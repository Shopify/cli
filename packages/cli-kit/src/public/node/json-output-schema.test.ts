...
interface Item {
  ...
  test('parenthesizes enum array elements to match the encoded result', () => {
    const outputSchema = defineJsonOutputSchema({
      name: 'Result',
      schema: zod.array(zod.enum(['a', 'b'])),
    })

    expect(outputSchema.typescript).toBe('type Result = ("a" | "b")[]')
    expect(outputSchema.validate(['a', 'b'])).toEqual(['a', 'b'])
  ...
  ] as const)('renders optional properties with %s wrappers', (_order, valueSchema) => {
    const outputSchema = defineJsonOutputSchema({
      name: 'Result',
      schema: zod.object({value: valueSchema}),
    })

    expect(outputSchema.typescript).toBe(`interface Result {
  value?: string | null
}`)
    expect(outputSchema.validate({})).toEqual({})
    expect(outputSchema.encode({})).toBe('{}')
    expect(outputSchema.validate({value: null})).toEqual({value: null})
    expect(outputSchema.validate({value: 'ready'})).toEqual({value: 'ready'})
  })
