 ...
 */
export function requestMode(preset: RequestModeInput = 'default'): RequestBehaviour {
  const networkLevelRetryIsSupported = !skipNetworkLevelRetry()
  switch (preset) {
    case 'default':
      return {
        useNetworkLevelRetry: networkLevelRetryIsSupported && true,
        maxRetryTimeMs: DEFAULT_MAX_TIME_MS,
        useAbortSignal: true,
        timeoutMs: maxRequestTimeForNetworkCallsMs(),
      }
    case 'non-blocking':
      return {
