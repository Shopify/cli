...
import {cliVersion, isPreReleaseVersion} from './version.js'
...
export async function runCLIUpgrade(): Promise<void> {
  ...
  if (isGlobal) {
    ...
    await upgradeLocalShopify(projectDir, cliVersion())
  } else {
...
export function versionToAutoUpgrade(): string | undefined {
  ...
  // Don't auto-upgrade for major version changes
  try {
    const currentSemVer = new SemVer(currentVersion)
    const newerSemVer = new SemVer(newerVersion)
    if (currentSemVer.major !== newerSemVer.major) {
      outputDebug('Auto-upgrade: Skipping auto-upgrade because of major version change.')
      return undefined
    }
    // eslint-disable-next-line no-catch-all/no-catch-all
  } catch {
    // If version parsing fails, skip auto-upgrade
    outputDebug('Auto-upgrade: Skipping auto-upgrade because of version parsing failure.')
    return undefined
  }
