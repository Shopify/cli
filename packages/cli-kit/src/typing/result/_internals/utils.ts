 ...
 */
export const combineResultList = <T, E>(resultList: ReadonlyArray<Result<T, E>>): Result<ReadonlyArray<T>, E> =>
  resultList.reduce(
    (acc, result) =>
      // eslint-disable-next-line no-nested-ternary
      acc.isOk() ? (result.isErr() ? err(result.error) : acc.map(appendValueToEndOfList(result.value))) : acc,
    ok([]) as Result<T[], E>,
  )
...
export const combineResultAsyncList = <T, E>(
  ...
  ResultAsync.fromSafePromise(Promise.all(asyncResultList)).andThen(combineResultList) as ResultAsync<T[], E>
...
export const combineResultAsyncListWithAllErrors = <T, E>(
  ...
  ResultAsync.fromSafePromise(Promise.all(asyncResultList)).andThen(combineResultListWithAllErrors) as ResultAsync<
    T[],
    E[]
  >
