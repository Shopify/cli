...
USAGE
  $ shopify theme duplicate
  $ shopify theme duplicate --theme 10 --rename 'New Theme'

FLAGS
  -e, --environment=<value>...  The environment to apply to the current command.
  -f, --force                   Force the duplicate operation to run without prompts or confirmations.
  -j, --json                    Output the result as JSON.
  -r, --rename=<value>          Rename the theme to the specified name.
  -s, --store=<value>           Store URL. It can be the store prefix (example) or the full myshopify.com URL
                                (example.myshopify.com, https://example.myshopify.com).
  -t, --theme=<value>           Theme ID or name of the remote theme.
      --no-color                Disable color output.
...
DESCRIPTION
  ...
  You can optionally rename the duplicated theme using the `--rename` flag.
