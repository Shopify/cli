...
export default class Demo extends Command {
  static summary = 'Demo command to showcase CLI App & Theme functionality'

  static description =
    'Shopify demo command to learn how to use the CLI for App & Theme development. Two current commands, app and theme to walk through each user flow.'
  ...
  async run(): Promise<void> {
    ...
    let demoType = flags.type

    if (!demoType) {
      demoType = await renderSelectPrompt({
...
async function themeDemo() {
  ...
  await inTemporaryDirectory(async (_tmpDir) => {
    ...
    await renderInfo({
      customSections: [
        {
          title: 'Well done running the development server!',
          body: {
            list: {
              items: [
                'This is an easy way to preview your theme changes on your store.',
                'Lets now push our changes to the store.',
              ],
            },
          },
        },
