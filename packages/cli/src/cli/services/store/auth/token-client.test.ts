import {exchangeStoreAuthCodeForToken, fetchCurrentStoreAuthScopes, refreshStoreAccessToken} from './token-client.js'
...
describe('token client', () => {
  ...
  test('refreshStoreAccessToken throws on malformed JSON', async () => {
    ...
    await expect(refreshStoreAccessToken({store: 'shop.myshopify.com', refreshToken: 'refresh-token'})).rejects.toThrow(
      'Received an invalid refresh response from Shopify.',
    )
  })
  ...
  test('refreshStoreAccessToken throws when access token is missing', async () => {
    ...
    await expect(refreshStoreAccessToken({store: 'shop.myshopify.com', refreshToken: 'refresh-token'})).rejects.toThrow(
      'Token refresh returned an invalid response for shop.myshopify.com.',
    )
  })
