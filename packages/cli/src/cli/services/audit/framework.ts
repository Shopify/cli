...
export abstract class AuditSuite {
  static description: string = 'Audit test suite'
  static requiresStore: boolean = false
  ...
  private getTestMethods(): string[] {
    ...
    while (proto && proto !== Object.prototype) {
      ...
      for (const name of names) {
        if (name.startsWith('test') && typeof (this as Record<string, unknown>)[name] === 'function') {
          methods.push(name)
        }
      }
   ...
   */
  protected async run(command: string, options?: {cwd?: string; env?: Record<string, string>}): Promise<CommandResult> {
  ...
  protected assertError(result: CommandResult, pattern?: RegExp | string, message?: string): void {
    ...
    if (pattern) {
      ...
      this.assertions.push({
        ...
        actual: failed ? (matches ? 'matched' : `output: ${result.output.slice(0, 200)}`) : 'command succeeded',
      })
    } else {
      this.assertions.push({
  ...
  protected async assertFile(
    ...
    if (!exists) {
      this.assertions.push({
        description: message ?? `File exists: ${path}`,
        passed: false,
    ...
    if (contentPattern) {
      ...
      this.assertions.push({
        description: message ?? `File ${path} matches ${contentPattern}`,
        passed: matches,
  ...
  protected async assertNoFile(path: string, message?: string): Promise<void> {
    ...
    const exists = await fileExists(fullPath)
    this.assertions.push({
      description: message ?? `File does not exist: ${path}`,
      passed: !exists,
  ...
  protected async assertDirectory(path: string, message?: string): Promise<void> {
    ...
    const exists = await fileExists(fullPath)
    this.assertions.push({
      description: message ?? `Directory exists: ${path}`,
      passed: exists,
...
/**
 * Type for a test suite class constructor
 */
export type AuditSuiteClass = new () => AuditSuite & {
  constructor: {
    description: string
