import {type CrawlerSignatureHeaders, type DevServerSession} from './types.js'
 ...
 * @param crawlerSignatureHeaders - Crawler signature headers sent with storefront requests.
 *
...
export async function initializeDevServerSession(
  ...
  const session = await fetchDevServerSession(
    themeId,
    adminSession,
    adminPassword,
    storefrontPassword,
  ...
  session.refresh = async () => {
    ...
    const newSession = await fetchDevServerSession(
      themeId,
      adminSession,
      adminPassword,
      storefrontPassword,
...
export async function fetchDevServerSession(
  ...
  const sessionCrawlerSignatureHeaders =
    crawlerSignatureHeaders ?? (await fetchOrCreateCrawlerSignatureHeaders(adminSession))
  ...
  const sessionCookies = await getStorefrontSessionCookiesWithVerification(
    ...
    storefrontToken,
    storefrontPassword,
    sessionCrawlerSignatureHeaders,
  )
