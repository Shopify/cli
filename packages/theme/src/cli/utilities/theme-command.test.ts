...
import * as pathUtils from '@shopify/cli-kit/node/path'
...
async function withThemeEnvironment(callback: (tmpDir: string) => Promise<void>) {
  await inTemporaryDirectory(async (tmpDir) => {
    vi.spyOn(pathUtils, 'cwd').mockReturnValue(tmpDir)
...
describe('ThemeCommand', () => {
  ...
  describe('run', () => {
    test('no environment provided', async () => {
      await withThemeEnvironment(async () => {
        // Given
    ...
    test('single environment provided', async () => {
      await withThemeEnvironment(async (tmpDir) => {
        // Given
    ...
    test('single environment provided without store - does not throw when store is not required', async () => {
      await withThemeEnvironment(async (tmpDir) => {
        // Given
        const environmentConfig = {path: joinPath(tmpDir, 'some/path')}
        await mkdir(environmentConfig.path)
        vi.mocked(loadEnvironment).mockResolvedValue(environmentConfig)
    ...
    test('single environment provided with store - does not create session when command does not require auth', async () => {
      await withThemeEnvironment(async (tmpDir) => {
        // Given
        const environmentConfig = {path: joinPath(tmpDir, 'some/path'), store: 'store.myshopify.com'}
        await mkdir(environmentConfig.path)
        vi.mocked(loadEnvironment).mockResolvedValue(environmentConfig)
    ...
    test("throws an AbortError if the path doesn't exist", async () => {
      await withThemeEnvironment(async (tmpDir) => {
        await CommandConfig.load()
        const nonExistentPath = joinPath(tmpDir, 'non-existent')
        vi.spyOn(pathUtils, 'cwd').mockReturnValue(nonExistentPath)
        const command = new TestThemeCommand([], CommandConfig)

        await expect(command.run()).rejects.toThrow(`Path does not exist: ${nonExistentPath}`)
      })
    })
  ...
  describe('multi environment', () => {
    ...
    test('commands should display an error if the --path flag is used', async () => {
      await withThemeEnvironment(async (tmpDir) => {
        ...
        vi.mocked(loadEnvironment).mockResolvedValue(environmentConfig)
        vi.mocked(renderConfirmationPrompt).mockResolvedValue(true)
        await writeFile(joinPath(tmpDir, 'shopify.theme.toml'), '')
      ...
      await withThemeEnvironment(async (tmpDir) => {
        ...
        vi.mocked(loadEnvironment)
          .mockResolvedValueOnce({
            ...
            theme: 'theme1.myshopify.com',
            path: joinPath(tmpDir, 'theme/path'),
          })
          .mockResolvedValueOnce({
            store: 'store2.myshopify.com',
            development: true,
            path: joinPath(tmpDir, 'development/path'),
          })
          .mockResolvedValueOnce({store: 'store3.myshopify.com', live: true, 'no-color': false})

        await mkdir(joinPath(tmpDir, 'theme/path'))
        await mkdir(joinPath(tmpDir, 'development/path'))
        ...
        expect(themeEnvFlags?.path).toEqual(resolvePath(joinPath(tmpDir, 'theme/path')))
        ...
        expect(developmentEnvFlags?.path).toEqual(resolvePath(joinPath(tmpDir, 'development/path')))
        ...
        expect(liveEnvFlags?.path).toEqual(tmpDir)
    ...
    test('multiple environment commands accept missing password when a store auth cache session exists', async () => {
      await withThemeEnvironment(async (tmpDir) => {
        ...
        vi.mocked(loadEnvironment)
          .mockResolvedValueOnce({store: 'store1.myshopify.com', path: joinPath(tmpDir, 'home/path/to/theme1')})
          .mockResolvedValueOnce({
            store: 'store2.myshopify.com',
            password: 'password2',
            path: joinPath(tmpDir, 'home/path/to/theme2'),
          })

        await mkdir(joinPath(tmpDir, 'home/path/to/theme1'))
        await mkdir(joinPath(tmpDir, 'home/path/to/theme2'))
