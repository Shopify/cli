...
describe('theme-fs', () => {
  ...
  describe('handleFileDelete', () => {
    ...
    test('emits unlink event immediately for non-liquid assets', async () => {
      // Given
      vi.mocked(fetchThemeAsset).mockResolvedValue({
        key: 'assets/base.css',
        value: 'file content',
      ...
      vi.mocked(deleteThemeAsset).mockImplementation(async () => {
        callOrder.push('deleteThemeAsset')
        return true
      })
      ...
      const deleteOperationPromise = new Promise<void>((resolve) => {
        themeFileSystem.addEventListener(
          'unlink',
          vi.fn(() => {
      ...
      // Then
      expect(callOrder).toEqual(['unlink', 'deleteThemeAsset'])
    })

    test('emits unlink event after remote deletion for liquid assets', async () => {
      // Given
      vi.mocked(fetchThemeAsset).mockResolvedValue({
        key: 'assets/base.css.liquid',
        value: 'file content',
        checksum: '1',
        stats: {size: 100, mtime: 100},
      })
      ...
      const watcher = chokidar.watch('') as EventEmitter
      watcher.emit('unlink', `${root}/assets/base.css.liquid`)

      await deleteOperationPromise

      // Then
      expect(callOrder).toEqual(['deleteThemeAsset', 'unlink'])
    })
  })
