...
import {duplicate, renderArgumentsWarning} from '../../services/duplicate.js'
...
export default class Duplicate extends ThemeCommand {
  ...
  static usage = ['theme duplicate', "theme duplicate --theme 10 --rename 'New Theme'"]
...
\`\`\``
  ...
  static flags = {
    ...
    rename: Flags.string({
      char: 'r',
      description: 'Rename the theme to the specified name.',
      env: 'SHOPIFY_FLAG_RENAME',
    }),
  ...
  async run(): Promise<void> {
    const {flags, argv} = await this.parse(Duplicate)
    ...
    if (argv.length > 0 && typeof argv[0] === 'string') {
      renderArgumentsWarning(argv[0], flags.json)
    }

    await duplicate(adminSession, themeId, flags)
  }
}
