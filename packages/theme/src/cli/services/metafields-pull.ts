...
import {renderError, renderInfo} from '@shopify/cli-kit/node/ui'
...
async function executeMetafieldsPull(session: AdminSession, options: MetafieldsPullOptions) {
  ...
  if (!(await hasRequiredThemeDirectories(path)) && !(await currentDirectoryConfirmed(force))) {
    return
  }
  ...
  if (!silent) {
    if (failedFetchByOwnerType.length > 0) {
      renderInfo({
        body: `Failed to fetch metafield definitions for the following owner types: ${failedFetchByOwnerType.join(
          ', ',
        )}`,
        nextSteps: ['Ensure you have the permission for each owner type to fetch its metafield definitions.'],
        reference,
      })
    }

    outputSuccess('Metafield definitions have been successfully downloaded.')
  }
}
