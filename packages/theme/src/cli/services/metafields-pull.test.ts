...
describe('metafields-pull', () => {
  ...
  test('should download metafields for each ownerType and write to file', async () => {
    ...
    expect(capturedOutput.success()).toContain('Metafield definitions have been successfully downloaded.')
    expect(capturedOutput.error()).toBeFalsy()
  })

  test('should render warning if some metafield definitions are not found', async () => {
    // Given
    vi.mocked(metafieldDefinitionsByOwnerType).mockImplementation((type: any, _session: AdminSession) => {
    ...
    await inTemporaryDirectory(async (tmpDir) => {
      ...
      expect(capturedOutput.success()).toContain('Metafield definitions have been successfully downloaded.')
      expect(capturedOutput.info()).toMatchInlineSnapshot(`
          "╭─ info ───────────────────────────────────────────────────────────────────────╮
          │                                                                              │
          ...
          │                                                                              │
          │  Next steps                                                                  │
          │    • Ensure you have the permission for each owner type to fetch its         │
          │      metafield definitions.                                                  │
          │                                                                              │
          │  Reference                                                                   │
          │    • Metafield Definition API [1]                                            │
          │                                                                              │
          ╰──────────────────────────────────────────────────────────────────────────────╯
          [1]
  ...
  test('should render error if no metafield definitions are found', async () => {
    ...
    await inTemporaryDirectory(async (tmpDir) => {
      ...
      expect(capturedOutput.success()).toBeFalsy()
