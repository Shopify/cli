...
import {renderConfirmationPrompt, renderError, renderSuccess, renderWarning} from '@shopify/cli-kit/node/ui'
...
export async function duplicate(adminSession: AdminSession, themeId: string | undefined, flags: DuplicateFlags) {
  const {rename, verbose, noColor, json, force} = flags
  const noPrompts = force || isCI()
  ...
  const result = await themeDuplicate(themeToDuplicate.id, rename, adminSession)
...
/**
 * Renders a deprecation warning for the theme positional argument.
 *
 * @param id - The theme ID that was passed as a positional argument.
 * @param json - Whether to output the warning in JSON format.
 */
export function renderArgumentsWarning(id: string, json: boolean) {
  if (json) {
    outputResult(
      JSON.stringify({
        message: `The theme ID positional argument is deprecated. Use the --theme flag instead:\n\n$ shopify theme duplicate --theme ${id}`,
        errors: [],
      }),
    )
  }
  ...
  renderWarning({
    body: [
      ...
      {command: '--theme'},
      'flag instead:\n\n',
      {command: `$ shopify theme duplicate --theme ${id}`},
      {char: '.'},
    ],
  })
}
