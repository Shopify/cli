...
import {duplicate, renderArgumentsWarning} from './duplicate.js'
...
describe('duplicate', () => {
  ...
  test('uses rename parameter when provided', async () => {
    // Given
    vi.mocked(isCI).mockReturnValue(true)
    vi.mocked(findThemeById).mockResolvedValue(theme)
    vi.mocked(themeDuplicate).mockResolvedValue({
    ...
    // When
    await duplicate(session, '1', {...options, rename: 'I have a new name now!'})
...
describe('renderArgumentsWarning', () => {
  test('renders warning', () => {
    // When
    renderArgumentsWarning('1', false)

    // Then
    expect(renderWarning).toHaveBeenCalled()
  })

  test('outputs JSON when using --json flag', () => {
    // When
    renderArgumentsWarning('1', true)
