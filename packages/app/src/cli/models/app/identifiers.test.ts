...
describe('getAppIdentifiers', () => {
  test('returns the right identifiers when variables are defined in the .env file', async () => {
    await inTemporaryDirectory(async (tmpDir: string) => {
      // Given
      const uiExtension = await testUIExtension({
        directory: '/tmp/project/extensions/my-extension-directory',
      ...
      const app = testApp({
        ...
        dotenv: {
          ...
          variables: {SHOPIFY_API_KEY: 'FOO', SHOPIFY_MY_EXTENSION_DIRECTORY_ID: 'BAR'},
        },
        allExtensions: [uiExtension],
      })
      ...
      expect((got.extensions ?? {})['my-extension-directory']).toEqual('BAR')
    })
  })
  ...
  test('returns the right identifiers when variables are defined in the system environment', async () => {
    await inTemporaryDirectory(async (tmpDir: string) => {
      ...
      const got = await getAppIdentifiers(
        ...
        {SHOPIFY_API_KEY: 'FOO', SHOPIFY_MY_EXTENSION_DIRECTORY_ID: 'BAR'},
      )
