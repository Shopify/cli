...
describe('all modules implement AppModule interface', () => {
  ...
  test('allAppModules contains all registered modules', () => {
    expect(allAppModules.length).toBeGreaterThanOrEqual(9)
    // Config modules (9)
    const configIds = allAppModules
      .filter((mod) => mod.tomlKeys !== undefined)
      .map((mod) => mod.identifier)
      .sort()
    ...
    // Non-config modules (extension types)
    const extensionIds = allAppModules
      .filter((mod) => mod.tomlKeys === undefined)
      .map((mod) => mod.identifier)
      .sort()
    expect(extensionIds).toContain('checkout_post_purchase')
    expect(extensionIds).toContain('function')
    expect(extensionIds).toContain('ui_extension')
  })
})
