...
class AppLoader {
  ...
  async loadUIExtensions(
    ...
    const extensions = configPaths.map(async (configurationPath) => {
      ...
      if (!spec) {
        this.abortOrReport(
          output.content`Couldn't find a valid specification for extension type ${output.token.yellow(type)}`,
          undefined,
          directory,
        )
        return undefined
  ...
  async loadFunctions(
    ...
    const functionInstances = configPaths.map(async (configurationPath) => {
      const directory = path.dirname(configurationPath)
      const fileContent = await file.read(configurationPath)
      const obj = toml.decode(fileContent)
      const {type} = TypeSchema.parse(obj)
      ...
      if (!spec) {
        this.abortOrReport(
          output.content`Couldn't find a valid specification for function type ${output.token.yellow(type)}`,
          undefined,
          directory,
        )
        return undefined
    ...
    const functions = getArrayRejectingUndefined(await Promise.all(functionInstances))
