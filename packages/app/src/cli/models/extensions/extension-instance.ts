...
import {joinPath, normalizePath, resolvePath, relativePath} from '@shopify/cli-kit/node/path'
...
export class ExtensionInstance<TConfiguration extends BaseConfigType = BaseConfigType> {
  ...
  get outputFileName() {
    return this.specification.getOutputFileName?.(this) ?? ''
  }
  ...
  constructor(options: {
    ...
    this.outputPath = joinPath(
      this.directory,
      this.specification.getOutputRelativePath?.(this) ?? '',
      this.outputFileName,
    )
  ...
  getOutputPathForDirectory(directory: string, outputId?: string) {
    ...
    return joinPath(directory, id, this.specification.getOutputRelativePath?.(this) ?? '', this.outputFileName)
  }
