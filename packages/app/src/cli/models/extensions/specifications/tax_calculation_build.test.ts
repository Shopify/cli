...
describe('tax_calculation buildConfig', () => {
  ...
  test('has a single create-tax-stub step', () => {
    const {steps} = taxCalculationSpec.clientSteps![0]!
  ...
  test('config is serializable to JSON', () => {
    const serialized = JSON.stringify(taxCalculationSpec.clientSteps!)
    const deserialized = JSON.parse(serialized)

    expect(deserialized[0].steps).toHaveLength(1)
    expect(deserialized[0].steps[0].type).toBe('create_tax_stub')
  })
