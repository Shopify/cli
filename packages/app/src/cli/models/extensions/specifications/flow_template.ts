...
const flowTemplateSpec = createExtensionSpecification({
  ...
  deployConfig: async (config, extensionPath) => {
    return {
      ...
      require_app: config.template.require_app || false,
      discoverable: config.template.discoverable || true,
      allow_one_click_activate: config.template.allow_one_click_activate || false,
      enabled: config.template.enabled || true,
