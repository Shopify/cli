...
export function convertApiVersionToSemver(apiVersion: string): string {
...
export function loadTsConfig(startPath: string): {compilerOptions: ts.CompilerOptions; configPath: string | undefined} {
...
export async function fallbackResolve(importPath: string, baseDir: string): Promise<string | null> {
...
export async function parseAndResolveImports(filePath: string): Promise<string[]> {
  try {
    await init
