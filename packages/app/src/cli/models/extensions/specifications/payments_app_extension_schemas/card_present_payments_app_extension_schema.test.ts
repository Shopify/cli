...
const config: CardPresentPaymentsAppExtensionConfigType = {
  ...
  terminal_transaction_result_url: 'http://foo.bar',
...
describe('CardPresentPaymentsAppExtensionSchema', () => {
  ...
  test('validates with optional terminal_transaction_result_url', async () => {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    const {terminal_transaction_result_url, ...rest} = config
  ...
  test('returns an error if terminal_transaction_result_url is not a valid URL', async () => {
    expect(() =>
      CardPresentPaymentsAppExtensionSchema.parse({
        ...config,
        terminal_transaction_result_url: 'not-a-url',
      }),
    ).toThrowError(
      new zod.ZodError([
        {
          ...
          code: 'invalid_string',
          message: 'Invalid url',
          path: ['terminal_transaction_result_url'],
        },
      ]),
    )
  })
...
describe('cardPresentPaymentsAppExtensionDeployConfig', () => {
  test('maps deploy configuration from extension configuration', async () => {
    ...
    expect(result).toMatchObject({
      ...
      terminal_transaction_result_url: config.terminal_transaction_result_url,
