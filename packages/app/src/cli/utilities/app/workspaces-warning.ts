...
export function showWorkspaceWarning(packageManager: PackageManager) {
  renderWarning({
    ...
    body: [
      'The CLI will soon stop installing dependencies in project subdirectories by default. To maintain the same behavior, please transition over to using workspaces by running',
      {command: formatPackageManagerCommand(packageManager, 'app migrate workspaces')},
    ],
    reference: [
      [
        {
