...
import {allLocalUISpecifications, allThemeSpecifications} from '../../models/extensions/specifications.js'
...
describe('fetchExtensionSpecifications', () => {
  it('returns the filtered and mapped results', async () => {
    ...
    const uilocal = await allLocalUISpecifications()
    const themelocal = await allThemeSpecifications()

    // When
    const got = await fetchExtensionSpecifications('token', 'apiKey', [...uilocal, ...themelocal])
