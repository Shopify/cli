...
import {PARTNERS_USER_SESSION, testOrganizationApp} from '../models/app/app.test-data.js'
...
describe('selectApp', () => {
  test('returns app if user selects one', async () => {
    ...
    // When
    const got = await selectAppPrompt(apps, ORG1.id, PARTNERS_USER_SESSION)
  ...
  test('includes toml names when present', async () => {
    ...
    const got = await selectAppPrompt(apps, ORG1.id, PARTNERS_USER_SESSION, {directory: '/'})
