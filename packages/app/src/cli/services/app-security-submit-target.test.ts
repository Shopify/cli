...
describe('resolveSecuritySubmitClientId', () => {
  test.each(['client_id = [', undefined])('uses an explicit client ID without loading config %s', async (content) => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await expect(resolveSecuritySubmitClientId({directory, clientId: 'explicit-client-id'})).resolves.toBe(
        'explicit-client-id',
      )
  ...
  test('reads the default linked config without requiring a complete app', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await expect(resolveSecuritySubmitClientId({directory})).resolves.toBe('default-client-id')
  ...
  test.each(['', ' \t '])(
    ...
    async (clientId) => {
      await inTemporaryDirectory(async (directory) => {
        ...
        const error = await resolveSecuritySubmitClientId({directory, clientId}).catch((error: unknown) => error)
  ...
  test.each(['production-eu', 'shopify.app.production-eu.toml', 'Production EU'])(
    ...
    async (configName) => {
      await inTemporaryDirectory(async (directory) => {
        ...
        await expect(resolveSecuritySubmitClientId({directory, configName})).resolves.toBe('named-client-id')
        expect(getCachedAppInfo).not.toHaveBeenCalled()
      })
    },
  )
  ...
  test('reads the cached config instead of the default config', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await expect(resolveSecuritySubmitClientId({directory})).resolves.toBe('cached-client-id')
    })
  })
  ...
  test('an explicit config overrides a stale cached config', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await expect(resolveSecuritySubmitClientId({directory, configName: 'production'})).resolves.toBe(
        'named-client-id',
      )
      expect(getCachedAppInfo).not.toHaveBeenCalled()
    })
  ...
  test('rejects a stale cached config without falling back to another app', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      const error = await resolveSecuritySubmitClientId({directory}).catch((error: unknown) => error)
  ...
  test.each([undefined, 'missing'])(
    ...
    async (configName) => {
      await inTemporaryDirectory(async (directory) => {
        ...
        const error = await resolveSecuritySubmitClientId({directory, configName}).catch((error: unknown) => error)
  ...
  test.each([
    ...
    await inTemporaryDirectory(async (directory) => {
      ...
      const result = resolveSecuritySubmitClientId({directory})
      if (clientId) {
