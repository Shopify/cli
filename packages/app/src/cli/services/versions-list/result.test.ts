import {renderAppVersionsListResult} from './result.js'
...
describe('renderAppVersionsListResult', () => {
  ...
  test('writes the typed JSON result without text presentation', async () => {
    const outputMock = mockAndCaptureOutput()
    ...
    expect(JSON.parse(outputMock.output())).toEqual([
      {
        message: 'message',
        versionTag: 'versionTag',
        status: 'active',
