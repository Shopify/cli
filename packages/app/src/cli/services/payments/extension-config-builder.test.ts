...
const translateDeployConfigKeyToCLI = (deployConfigKey: string): string => {
  switch (deployConfigKey) {
    ...
    case 'default_buyer_label':
      return 'buyer_label'
    case 'buyer_label_to_locale':
      return 'buyer_label_translations'
    case 'encryption_certificate':
      return 'encryption_certificate_fingerprint'
    default:
      return deployConfigKey
  }
}

const expectObjectIncludesKeys = (got: object, config: string) => {
  const configObj = JSON.parse(config)
  const gotObj = got as {extensions: {[key: string]: unknown}[]}
  const ext = gotObj.extensions[0]!
  const topLevel = got as {[key: string]: unknown}
  ...
  for (const key of keys) {
    ...
    const foundInExt = translatedKey in ext
    const foundInTop = translatedKey in topLevel
    expect(foundInExt || foundInTop).toBe(true)
  }
}
      ...
      extensions: [
        {
          ...
          verification_session_url: undefined,
          encryption_certificate_fingerprint: 'fingerprint',
          checkout_payment_method_fields: undefined,
      ...
      extensions: [
        {
          ...
          supported_buyer_contexts: [{currency: 'EUR'}],
          buyer_label: null,
          buyer_label_translations: null,
          encryption_certificate_fingerprint: 'Test Certificate',
          test_mode_available: true,
          multiple_capture: false,
          checkout_payment_method_fields: [{key: 'payment_plan', type: 'string', required: true}],
      ...
      extensions: [
        {
          ...
          supported_buyer_contexts: undefined,
          test_mode_available: true,
          multiple_capture: false,
          buyer_label: 'Bogus Pay Buyer Label',
          buyer_label_translations: [],
          checkout_payment_method_fields: [{key: 'bogus_customer_document', type: 'string', required: true}],
      ...
      extensions: [
        {
          ...
          supported_buyer_contexts: [{currency: 'USD', countries: ['US']}, {currency: 'GBP'}],
          test_mode_available: true,
          buyer_label: null,
          buyer_label_translations: null,
          ...
          checkout_payment_method_fields: [
            {key: 'card_number', type: 'string', required: true},
            {key: 'pin', type: 'string', required: true},
          ],
          ui_extension_handle: 'checkout-ui-extension',
      ...
      extensions: [
        {
          ...
          sync_terminal_transaction_result_url:
            'https://bogus-payment-sessions.shopifycloud.com/bogus/card_present/terminal_transaction_result',
