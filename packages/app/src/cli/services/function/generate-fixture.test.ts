...
const mockLoadConfigurationFileContent = vi.mocked(loader.loadConfigurationFileContent)
...
describe('generateFixture', () => {
  ...
  beforeEach(() => {
    ...
    mockExistsSync.mockImplementation((path) => {
      // Default to true, but return false for TOML file to skip TOML parsing
      if (path === '/test/path/shopify.extension.toml') return false
      return true
    })
    ...
    mockLoadConfigurationFileContent.mockResolvedValue({} as any)
  ...
  test('generates test fixture with default test file', async () => {
    ...
    mockReadFile.mockImplementation((path) => {
      if (path === '/packages/app/templates/function/default.test.ts.template') {
    ...
    expect(mockReadFile).toHaveBeenCalledWith('/packages/app/templates/function/default.test.ts.template')
  ...
  test('uses existing test directories when they exist', async () => {
    ...
    expect(mockReadFile).not.toHaveBeenCalledWith('/packages/app/templates/function/default.test.ts.template')
  ...
  test('throws error when selector prompt returns undefined', async () => {
    ...
    }).rejects.toThrow('No function run selected. Exiting.')
  })
  ...
  test('throws error when no function run files are found', async () => {
    mockReaddirSync.mockReturnValue([] as any)
