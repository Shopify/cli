...
import {groupIssues, type IssueGroup, Capabilities, Issue, ScanResult, Severity} from './app-doctor-engine/index.js'
...
// A triage hint for widespread repetition, not a severity or scoring threshold.
const SYSTEMIC_PATTERN_FILE_THRESHOLD = 20
...
export function buildDoctorAlert(input: DoctorReportInput): DoctorAlert {
  ...
  return {
    type,
    options: {
      headline: doctorHeadline(input, groups),
      ...
      customSections: doctorCustomSections(input, groups),
    },
  }
}
...
function doctorHeadline(input: DoctorReportInput, groups: IssueGroup[]): string {
...
function doctorCustomSections(input: DoctorReportInput, groups: IssueGroup[]): AlertCustomSection[] {
  ...
  const systemic = groups.filter((group) => group.files.length >= SYSTEMIC_PATTERN_FILE_THRESHOLD)
  if (systemic.length > 0) {
    sections.push({
      title: 'Systemic patterns',
      body: {
        list: {
          items: systemic.map((group) => [
            {bold: group.issues[0]!.title},
            `${group.issues.length} occurrences across ${group.files.length} files.`,
            'Check for a shared helper or boundary and fix it there, rather than only patching each caller.',
          ]),
        },
      },
    })
