...
export interface AppSecurityFindings {
  accepted: number
  rejected: string[]
  warnings: string[]
}
...
export interface AppSecurityCompile {
  operation: 'compile'
  appRoot: string
  scan: ScanResult
  trace: TraceV2
...
export function parseFindings(value: unknown): FindingsDocument {
  if (!value || typeof value !== 'object') {
    throw new FindingsDocumentError('The App Security findings file must contain a JSON object.')
  }
  if (!('schema_version' in value) || value.schema_version !== FINDINGS_SCHEMA_VERSION) {
    throw new FindingsDocumentError(
      `The App Security findings file must use schema version ${FINDINGS_SCHEMA_VERSION}.`,
  ...
  if (
    ...
    throw new FindingsDocumentError(
      'The App Security findings file must identify its source scan.',
      'Copy the source_scan_id from the generated review.json.',
    )
  }
  if (!('findings' in value) || !Array.isArray(value.findings)) {
    throw new FindingsDocumentError('The App Security findings file must contain a findings array.')
  }
  if ('suppressions' in value && value.suppressions !== undefined && !Array.isArray(value.suppressions)) {
    throw new FindingsDocumentError('The App Security findings file suppressions field must be an array.')
  }

  return value as FindingsDocument
}

export async function scanApp(directory?: string): Promise<AppSecurityScan> {
...
export async function compileFindings(directory: string, document: FindingsDocument): Promise<AppSecurityCompile> {
  const appRoot = findAppRoot(directory)
  const result = await scan(appRoot)
  const engineVersion = getEngineVersion()
