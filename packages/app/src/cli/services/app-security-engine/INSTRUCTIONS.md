...
Read `.shopify/app-security/submission.json` before uploading. Select the intended app with `--config <name>` or `--client-id <id>` as needed, using the same selection for inspection and upload.
