...
import {resolveVersionDiffExtensionIdentifiersBreakdown} from './context/identifiers.js'
...
export async function release(options: ReleaseOptions) {
  ...
  const {versionDetails} = await versionDiffByVersion(partnersApp.apiKey, options.version, token)

  const extensionIdentifiersBreakdown = await resolveVersionDiffExtensionIdentifiersBreakdown(
    token,
    partnersApp.apiKey,
    options.version,
