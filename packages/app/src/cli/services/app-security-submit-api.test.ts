...
describe('submitAppSecurityScan', () => {
  test('preserves multiple upload-URL user errors in server order and does not upload', async () => {
    ...
    await expect(submitAppSecurityScan(input, dependencies(upload))).resolves.toEqual({
      status: 'failed',
  ...
  test('returns expected PUT failures and does not create a source scan', async () => {
    ...
    await expect(submitAppSecurityScan(input, dependencies(upload))).resolves.toMatchObject({
      status: 'failed',
  ...
  test.each([false, true])('preserves create user errors in server order and accepted=%s', async (accepted) => {
    ...
    await expect(submitAppSecurityScan(input, dependencies())).resolves.toEqual({
      status: 'failed',
      error: {
        stage: 'create',
  ...
  test('retains the create fallback when a user error has an empty message', async () => {
    ...
    await expect(submitAppSecurityScan(input, dependencies())).resolves.toMatchObject({
      status: 'failed',
      error: {
        message: 'Shopify could not create the App Security scan.',
  ...
  test('returns a retry suggestion when Shopify does not accept the submission', async () => {
    ...
    const result = submitAppSecurityScan(input, dependencies())

    await expect(result).resolves.toEqual({
      status: 'failed',
      error: {
        stage: 'create',
        message: 'Shopify did not accept the App Security submission.',
        tryMessage: 'Try submitting the App Security results again.',
        userErrors: [],
        accepted: false,
      },
  ...
  test.each(['upload-url', 'upload', 'create'] as const)('propagates unknown %s errors as bugs', async (stage) => {
    ...
    await expect(submitAppSecurityScan(input, {upload})).rejects.toBe(bug)
  })
