...
import webhooksExtension from '../../models/templates/webhooks_extension.js'
...
export async function fetchExtensionTemplates(
  ...
  const localTemplates = localExtensionTemplates().filter((template) => !remoteIDs.includes(template.identifier))
  const allTemplates = remoteTemplates.templateSpecifications.concat(localTemplates)
...
export function localExtensionTemplates(): ExtensionTemplate[] {
  return [themeExtension, productSubscriptionUIExtension, webPixelUIExtension, webhooksExtension]
}
