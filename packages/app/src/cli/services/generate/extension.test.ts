  ...
  it(
    ...
    async () => {
      await withTemporaryApp(async (tmpDir) => {
        ...
        const generatedExtension = (await loadApp(tmpDir, allSpecs)).extensions.ui[0]!
  ...
  it(
    ...
    async () => {
      await withTemporaryApp(async (tmpDir) => {
        ...
        const loadedApp = await loadApp(tmpDir, allSpecs)
  ...
  it(
    ...
    async () => {
      await withTemporaryApp(async (tmpDir: string) => {
        ...
        await createFromTemplate({name, specification, extensionFlavor, appDirectory: tmpDir, allSpecs})
        await expect(
          createFromTemplate({name, specification, extensionFlavor, appDirectory: tmpDir, allSpecs}),
...
async function createFromTemplate({
  ...
  await extensionInit({
    name,
    specification,
    app: await loadApp(appDirectory, allSpecs),
    cloneUrl: 'cloneurl',
    extensionFlavor,
