...
describe('formatBulkOperationCancellationResult', () => {
  test.each([
    {
      ...
      headline: 'Bulk operation cancellation initiated.',
      body: 'The operation is being canceled. This may take a few moments.',
      renderType: 'success',
    },
    {
  ...
  ])('formats $status status with $renderType render type', ({status, headline, body, renderType}) => {
  ...
  test('includes operation details in custom sections', () => {
    ...
    expect(items.some((item) => item.includes('CANCELING'))).toBe(true)
  ...
  test('does not include completedAt when operation is not finished', () => {
    const operation = createMockOperation({status: 'CANCELING', completedAt: null})
    const result = formatBulkOperationCancellationResult(operation)
