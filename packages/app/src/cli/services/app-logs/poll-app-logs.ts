...
export interface AppEventData {
  ...
  payload: FunctionPayload | ArbitraryPayload
  event_type: string
  source: string
      ...
      for (const log of appLogs) {
        ...
        await useConcurrentOutputContext({outputPrefix: log.source}, async () => {
          if (log.event_type === 'function_run') {
            const functionPayload = payload as FunctionPayload
            const fuel = (functionPayload.fuel_consumed / ONE_MILLION).toFixed(4)

            if (log.status === 'success') {
              stdout.write(`Function executed successfully using ${fuel}M instructions.`)
            } else if (log.status === 'failure') {
              stdout.write(`❌ Function failed to execute with error: ${functionPayload.error_type}`)
            }

            const logs = functionPayload.logs
            if (logs.length > 0) {
              stdout.write(logs)
            }
          } else {
