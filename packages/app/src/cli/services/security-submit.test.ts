...
function testDependencies(directory: string): SecuritySubmitDependencies {
  return {
...
async function capturedAbort(run: Promise<unknown>): Promise<AbortError> {
  try {
    await run
    throw new Error('Expected securitySubmit to throw')
...
function expectNoOutput(dependencies: SecuritySubmitDependencies): void {
...
describe('securitySubmit', () => {
  test.each([false, true])('accepts reports above the former 1 MiB cap (dryRun=%s)', async (dryRun) => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await securitySubmit({...options(directory), force: true, dryRun}, dependencies)

      const bytes = vi.mocked(dependencies.writeSubmission).mock.calls[0]![1]
      expect(bytes.length).toBe(byteSize)
      await expect(readFile(appSecurityArtifactPaths(directory).submissionPath)).resolves.toBe(bytes.toString())
  ...
  test('does not upload if writing the final feedback payload fails', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await expect(securitySubmit(options(directory), dependencies)).rejects.toThrow('Disk full')

      expect(dependencies.submitScan).not.toHaveBeenCalled()
      await expect(readFile(appSecurityArtifactPaths(directory).submissionPath)).resolves.toContain('"feedback": null')
    })
  })
  ...
  test('does not upload changes made to the artifact during confirmation', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await securitySubmit({...options(directory), feedback: 'Approved feedback'}, dependencies)
      ...
      await expect(readFile(appSecurityArtifactPaths(directory).submissionPath)).resolves.toBe('tampered artifact')
    })
  })
  ...
  test('preserves explicit feedback until payload preparation and bypasses the prompt', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await securitySubmit({...options(directory), force: true, feedback: '  Direct feedback  '}, dependencies)
  ...
  test.each([
    ...
    await inTemporaryDirectory(async (directory) => {
      ...
      await securitySubmit({...options(directory), force: true, feedback: '-'}, dependencies)
  ...
  test('errors actionably when --feedback - has no piped stdin', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      const error = await capturedAbort(
        securitySubmit({...options(directory), force: true, feedback: '-'}, dependencies),
      )
  ...
  test.each([
    ...
    await inTemporaryDirectory(async (directory) => {
      ...
      await securitySubmit({...options(directory), force: true, dryRun, feedback: input}, dependencies)
  ...
  test.each([
    ...
    await inTemporaryDirectory(async (directory) => {
      ...
      await securitySubmit({...options(directory), ...overrides}, dependencies)
  ...
  test.each(['', ' \t '])('dry-run rejects an explicit blank client ID %j', async (clientId) => {
    await inTemporaryDirectory(async (directory) => {
      const dependencies = testDependencies(directory)
      vi.mocked(dependencies.resolveClientId).mockImplementation(resolveSecuritySubmitClientId)

      const error = await capturedAbort(securitySubmit({...options(directory), dryRun: true, clientId}, dependencies))
  ...
  test('dry run writes explicit feedback into the exact payload without prompting', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await securitySubmit({...options(directory), dryRun: true, feedback: '  Dry-run feedback  '}, dependencies)
  ...
  test.each([
    ...
    async ({feedback, expectedFeedback}) => {
      await inTemporaryDirectory(async (directory) => {
        ...
        await securitySubmit({...options(directory), feedback}, dependencies)
  ...
  test('--json --dry-run does not require --force and returns the payload artifact', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      const result = await securitySubmit({...options(directory), json: true, dryRun: true}, dependencies)
  ...
  test('--force skips prompting even when prompting is unavailable', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      const result = await securitySubmit({...options(directory), force: true}, dependencies)
    ...
    test.each([
      ...
      async ({clientId, configName, configFile, expectedClientId}) => {
        await inTemporaryDirectory(async (directory) => {
          ...
          await securitySubmit({...options(directory), json, force: true, clientId, configName}, dependencies)
    ...
    test.each([undefined, 'name = "unlinked-app"'])(
      ...
      async (configContent) => {
        await inTemporaryDirectory(async (directory) => {
          ...
          const error = await capturedAbort(securitySubmit({...options(directory), json, force: true}, dependencies))
  ...
  test('resolves the target before writing the finalized payload, then fetches the remote app', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      vi.mocked(dependencies.fetchApp).mockImplementationOnce(async (clientId) => {
        const writtenBytes = vi.mocked(dependencies.writeSubmission).mock.calls[0]![1]
        await expect(readFile(appSecurityArtifactPaths(directory).submissionPath)).resolves.toBe(
  ...
  test('--json without --force fails before reading inputs, writing artifacts, or resolving the target', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      const error = await capturedAbort(securitySubmit({...options(directory), json: true}, dependencies))
  ...
  test('returns API failure data unchanged for the command to render', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await expect(securitySubmit({...options(directory), force: true}, dependencies)).resolves.toBe(failure)
      expectNoOutput(dependencies)
    })
  })
  ...
  test('propagates expected local submission errors for the command to handle', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      await expect(securitySubmit({...options(directory), force: true}, dependencies)).rejects.toBe(failure)
  ...
  test('--json --force returns submission data and forwards metadata', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      const result = await securitySubmit(
        {
          ...options(directory),
          json: true,
          force: true,
