...
export function getSubscriptionLinkExtensionsToMigrate(
  localSources: LocalSource[],
  remoteSources: RemoteSource[],
  identifiers: IdentifiersExtensions,
) {
  const ids = getExtensionIds(localSources, identifiers)
  const localExtensionTypesToMigrate = ['subscription_link_extension']
  const remoteExtensionTypesToMigrate = ['subscription_link']
  const typesMap = new Map<string, string>([['subscription_link_extension', 'subscription_link']])
