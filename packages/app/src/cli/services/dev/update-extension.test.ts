...
describe('updateExtensionDraft()', () => {
  ...
  test('uses manifest main path for generic UI extension serialized script', async () => {
    ...
    await inTemporaryDirectory(async (tmpDir) => {
      ...
      const config = JSON.parse(updateCall.config as string)
      expect(config.serialized_script).toBe(Buffer.from('manifest content').toString('base64'))
    })
  })

  test('falls back to output path when generic UI extension manifest main path escapes the build directory', async () => {
    const developerPlatformClient: DeveloperPlatformClient = testDeveloperPlatformClient()
    await inTemporaryDirectory(async (tmpDir) => {
      const target = 'purchase.checkout.block.render'
      const configuration = {
      ...
      await writeFile(joinPath(tmpDir, 'uid1', 'manifest.json'), JSON.stringify({[target]: {main: '../outside.js'}}))
      await writeFile(joinPath(tmpDir, 'outside.js'), 'outside content')
      await writeFile(mockExtension.getOutputPathForDirectory(tmpDir), 'fallback content')
