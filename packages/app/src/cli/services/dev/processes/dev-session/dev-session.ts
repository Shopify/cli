...
export class DevSession {
  ...
  private async uploadAssetsIfNeeded(assets: string[], bundleController: AbortController): Promise<string | undefined> {
    ...
    await this.createZipFile(bundleZipPath, assets)
  ...
  /**
   * Create a zip file with the extensions
   * @param bundleZipPath - The path to the zip file
   */
  private async createZipFile(bundleZipPath: string, assets: string[]) {
    const blob = `{${assets.map((ext) => `${ext}/**`).join(',')},manifest.json}`
    await zip({
      inputDirectory: this.bundlePath,
      outputZipPath: bundleZipPath,
      matchFilePattern: blob,
    })
  }
