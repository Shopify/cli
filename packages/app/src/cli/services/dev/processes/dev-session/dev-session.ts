    ...
    if (devSession && this.currentSessionState) {
      ...
      if (devSession.websocketUrl !== expectedWebsocketUrl || devSession.user?.id !== expectedUserId) {
        ...
        await this.logger.error(
          `\n⚠️  Another developer ${newUserEmail ? `(${newUserEmail}) ` : ''}has taken over this dev preview.\n` +
            `Your preview is no longer active. Terminating dev preview...`,
        )

        // Throw AbortError to terminate gracefully
        const message = `Dev preview has been taken over by another user${newUserEmail ? ` (${newUserEmail})` : ''}.`
