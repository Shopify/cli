...
export async function executeStepByType(step: BuildStep, context: BuildContext): Promise<unknown> {
  switch (step.type) {
    ...
    case 'custom':
      throw new Error(
        `Build step type "${step.type}" is not yet implemented. Only "copy_files" is currently supported.`,
      )

    default:
      // This should never happen due to TypeScript type checking, but provides a runtime safety net
      throw new Error(`Unknown build step type: ${(step as BuildStep).type}`)
  }
}
