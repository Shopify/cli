...
describe('build_steps integration', () => {
  ...
  test('handles optional source that does not exist', async () => {
    await inTemporaryDirectory(async (tmpDir) => {
      const extensionDir = joinPath(tmpDir, 'extension')
      const outputDir = joinPath(tmpDir, 'output')
      ...
      // Don't create the 'optional-assets' directory
      ...
      // Should not throw even though source doesn't exist
      await expect(executeBuildSteps(mockExtension, stepsConfig, buildOptions)).resolves.not.toThrow()
    })
  })

  test('skips step when skip condition returns true', async () => {
    await inTemporaryDirectory(async (tmpDir) => {
      const extensionDir = joinPath(tmpDir, 'extension')
      const assetsDir = joinPath(extensionDir, 'assets')
      const outputDir = joinPath(tmpDir, 'output')
      ...
      await writeFile(joinPath(assetsDir, 'file.txt'), 'content')
      ...
      // Verify: File was not copied because step was skipped
      const fileExistsResult = await fileExists(joinPath(outputDir, 'file.txt'))
      expect(fileExistsResult).toBe(false)
    })
  })
})
