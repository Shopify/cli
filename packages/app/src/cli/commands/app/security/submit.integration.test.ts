...
vi.mock('../../../services/app-security-submit-target.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../../services/app-security-submit-target.js')>()
  return {...actual, resolveSecuritySubmitClientId: vi.fn(actual.resolveSecuritySubmitClientId)}
})
...
async function runCommand(argv: string[]) {
  ...
  try {
    ...
    await SecuritySubmit.run(argv, config)
...
describe('app security submit command boundary', () => {
  ...
  describe.each([false, true])('dry-run rejects invalid explicit config (json=%s)', (json) => {
    test.each([
      ...
      await inTemporaryDirectory(async (directory) => {
        ...
        expect(resolveSecuritySubmitClientId).toHaveBeenCalledOnce()
  ...
  test('API failure uses the real expected human error handler and retry help', async () => {
    await inTemporaryDirectory(async (directory) => {
      ...
      expect(result.exitCode).toBe(1)
      expect(result.stdout).toBe('')
      expect(result.stderr).toContain('Shopify did not accept the App Security submission.')
      expect(result.stderr).toContain('Try submitting the App Security results again.')
      expect(result.stderr).not.toContain('To investigate the issue, examine this stack trace:')
    })
  })
  ...
  test.each(['preparation', 'upload'] as const)('human FetchError at %s is expected', async (stage) => {
    await inTemporaryDirectory(async (directory) => {
      ...
      expect(result.exits).toEqual([1])
      expect(result.stdout).toBe('')
      expect(result.stderr).toContain('A network error interrupted the App Security submission.')
