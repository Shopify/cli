...
describe('migration submission JSON contract', () => {
  test('preserves an empty successful submission without failure details', () => {
    ...
    expect(encodeMigrationSubmissionResult({status: 'success', submission: value})).toBe(
  ...
  test('preserves total submission failure with nullable error fields', () => {
    ...
    expect(encodeMigrationSubmissionResult({status: 'failed', submission: value, failure})).toBe(
      JSON.stringify({schemaVersion: 1, ...value, failure}, null, 2),
    )
  })
  ...
  test.each([
    ...
    expect(() => migrationSubmissionJsonOutputSchema.validate({schemaVersion: 1, ...submission(), ...fields})).toThrow()
  })
})
...
describe('unschedule JSON compatibility', () => {
  test('uses the shared submission contract and preserves the unschedule action', () => {
    ...
    expect(encoded).toBe(JSON.stringify({schemaVersion: 1, ...value}, null, 2))
    expect(migrationSubmissionJsonOutputSchema.validate(JSON.parse(encoded))).toEqual({schemaVersion: 1, ...value})
