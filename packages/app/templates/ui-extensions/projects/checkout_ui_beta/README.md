...
└── my-app
  └── extensions
    └── my-checkout-ui-extension
        ├── src
        │   └── AdminCheckoutEditor.[js|ts|jsx|tsx] The source code for the ADMIN::CHECKOUT::EDITOR extension point
        │   └── CheckoutDynamicRender.[js|ts|jsx|tsx] The source code for the CHECKOUT:DYNAMIC::RENDER extension point
        ├── locales
...
You can customize your new extension by editing the code in the `src/AdminCheckoutEditor.[js|ts|jsx|tsx]` or `src/CheckoutDynamicRender.[js|ts|jsx|tsx]` file.
...
module = "./src/CheckoutCartLinesRenderAfter.[js|ts|jsx|tsx]"
````

You would then add a new file at `src/CheckoutCartLinesRenderAfter.[js|ts|jsx|tsx]`.
