...
export function renderAsyncOperationStarted(
  operation: string,
  ...
  outputInfo(
    colors.dim(` • Continue watching with `) + colors.dim(colors.bold(`shopify store copy show ${compositeId} --watch`)),
  )
  outputInfo(`${colors.dim(` • View details with `) + colors.dim(colors.bold(`shopify store copy show ${compositeId}`))}\n`)
}
