...
describe('listStoreAuthSessions', () => {
  test('projects stored store auth summaries into the typed result contract', () => {
    ...
    expect(JSON.parse(storeAuthListJsonOutputSchema.encode(result))).toEqual(result)
  })

  test('omits unavailable expiry and user details without changing the existing session fields', () => {
    vi.mocked(listStoredStoreAuthSummaries).mockReturnValue([
      {store: 'shop.myshopify.com', userId: '42', scopes: [], acquiredAt: '2026-03-27T00:00:00.000Z'},
    ])

    expect(JSON.parse(storeAuthListJsonOutputSchema.encode(listStoreAuthSessions()))).toEqual({
      sessions: [
        {
          subdomain: 'shop',
          connected: 'Mar 27, 2026',
