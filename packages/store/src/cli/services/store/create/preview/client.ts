...
function previewStoreError(status: number, rawText: string): {message: string; tryMessage?: string} {
  ...
  if (errorCode === 'dependency_unavailable') {
    return {message: 'Preview store creation is temporarily unavailable.', tryMessage: 'Try again in a few minutes.'}
  }
  ...
  return {
    ...
    tryMessage: message ?? (rawText.length > 0 ? rawText.slice(0, 1000) : 'No response body returned.'),
  }
}
