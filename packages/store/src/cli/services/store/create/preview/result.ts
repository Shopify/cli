  ...
  return JSON.stringify(
    {
      ...
      adminDomain: result.adminDomain,
      storefrontDomain: result.storefrontDomain,
...
function renderTextResult(result: CreatePreviewStoreResult): void {
  renderSuccess({
        ...
        body: {
          list: {
            items: [
              ...
              `Admin API domain: ${result.adminDomain}`,
              `Storefront domain: ${result.storefrontDomain}`,
    ...
    nextSteps: [
      [
        ...
        {
          command: `shopify store execute --store ${result.adminDomain} --query '{ shop { name } }'`,
        },
      ],
  ...
  outputResult(
    outputContent`Preview store admin token stored locally and accessible via ${outputToken.genericShellCommand(
      `shopify store execute --store ${result.adminDomain}`,
    )}.`.value,
  )
}
