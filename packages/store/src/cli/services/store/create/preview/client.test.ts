...
const validBody = {
  shop_id: 21,
  shop_permanent_domain: 'preview-1.my.shop.dev',
  ...
  store_auth_bootstrap: {
    access_token: 'shpat_preview_token',
    scopes: ['read_themes', 'write_themes'],
    api_key: 'development-shop-merchant-key',
    shop_domain: 'preview-1.dev-api.shop.dev',
  },
}
...
describe('createPreviewStore', () => {
  ...
  test('rejects responses missing required identifier fields', async () => {
    vi.mocked(shopifyFetch).mockResolvedValue(fakeOkResponse({...validBody, store_auth_bootstrap: undefined}))
