...
export async function diffWorkflow(input: DiffInput): Promise<DiffResult> {
  ...
  if (!workflowId) {
    throw new AbortError(
      ...
      'Run `flow workflow push` first, or pass --workflow-id.',
    )
  }
...
export function unifiedDiff(remoteLabel: string, remoteText: string, localLabel: string, localText: string): string {
  ...
  while (i < remoteLines.length || j < localLines.length) {
    if (lcsIdx < lcs.length && remoteLines[i] === lcs[lcsIdx] && localLines[j] === lcs[lcsIdx]) {
      lines.push(` ${remoteLines[i]}`)
      i++
      j++
      lcsIdx++
    } else if (j < localLines.length && (lcsIdx >= lcs.length || localLines[j] !== lcs[lcsIdx])) {
      lines.push(`+${localLines[j]}`)
      j++
    } else if (i < remoteLines.length && (lcsIdx >= lcs.length || remoteLines[i] !== lcs[lcsIdx])) {
      lines.push(`-${remoteLines[i]}`)
      i++
    }
  }
...
function longestCommonSubsequence(seqA: string[], seqB: string[]): string[] {
  const dp: number[][] = Array.from({length: seqA.length + 1}, () => new Array(seqB.length + 1).fill(0))
