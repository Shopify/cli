...
function unwrapTemplateLookupResult(response: unknown): TemplateLookupResponseJson {
...
export default class FlowTemplateSave extends Command {
...
Get a template ID from \`flow template search\` — each result carries a stable \`template_id\` field.`
  ...
  public async run(): Promise<void> {
    ...
    if (!dir) {
      throw new AbortError('No workflows directory. Pass --workflows-dir, or run `flow init` to create a flow.toml.')
    }
    ...
    const json = unwrapTemplateLookupResult(response)
    if (json.error) {
    ...
    const lines = [
      ...
      '',
      'Next:',
      `  flow workflow validate ${filePath}`,
      `  flow workflow push     ${filePath}     # creates lockfile`,
    ]
