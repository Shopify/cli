...
export default class FlowWorkflowStatus extends Command {
...
Exits 1 if anything other than \`clean\` is present.`
  ...
  public async run(): Promise<void> {
    ...
    if (!dir) {
      throw new AbortError('No workflows directory. Pass --workflows-dir, or run `flow init`.')
    }
...
function formatHuman(items: StatusItem[], counts: Record<WorkflowStatus, number>, store: string, dir: string): string {
  const header = [
    `Store:           ${store}`,
    `Workflows dir:   ${dir}`,
    '',
