...
export default class FlowWorkflowPull extends Command {
...
With \`--all\`: lists every workflow on the shop and writes one file per workflow into the project's workflows directory. Skips files that already exist unless \`--force\` is set. Requires \`flow.toml\` (run \`flow init\` first), or pass \`--workflows-dir\`.`
  ...
  public async run(): Promise<void> {
    ...
    if (!flags['workflow-id'] || !flags.out) {
      throw new AbortError('Pull a single workflow with --workflow-id and --out, or pull every workflow with --all.')
    }

    const store = flags.store ?? (await loadConfig())?.store
    if (!store) {
      throw new AbortError('No store. Pass --store, or run `flow init` to write a flow.toml.')
    }
    ...
    if (flags.json) {
      ...
      outputResult(
        `Pulled workflow ${result.workflowId} (version ${result.workflowDefinitionVersion}) to ${flags.out}.`,
      )
    }
  }
  ...
  private async runPullAll(flags: {
    ...
    if (!store) {
      throw new AbortError('--all requires a store. Pass --store or run `flow init` to create a flow.toml.')
    }

    let dir: string | undefined
    if (flags['workflows-dir']) {
      dir = resolvePath(cwd(), flags['workflows-dir'])
    } else if (config) {
      dir = workflowsDirAbsolute(config)
    }
    if (!dir) {
      throw new AbortError('--all requires a workflows directory. Pass --workflows-dir or run `flow init`.')
    }
