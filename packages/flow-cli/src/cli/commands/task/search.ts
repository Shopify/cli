...
export default class FlowTaskSearch extends Command {
...
Returns task references (id + version) you can pass to \`flow task describe\` to get the full configuration schema.`
  ...
  static flags = {
    ...
    type: Flags.string({
      ...
      options: [...TASK_TYPES],
      env: 'SHOPIFY_FLAG_FLOW_TASK_TYPE',
    }),
  }
...
function formatHuman(result: SearchResult): string {
  ...
  const lines = [
    ...
    ...result.tasks.map(
      (task) =>
        `${task.task_type.padEnd(typeWidth)}  ${(task.publisher ?? '?').padEnd(pubWidth)}  ${`${task.id}@${task.version}`.padEnd(
          idWidth,
        )}  ${task.label.padEnd(labelWidth)}`,
    ),
  ]
