...
describe('getDeployConfig()', () => {
  it(
    ...
    async () => {
      await file.inTemporaryDirectory(async (tmpDir) => {
        console.log('[Extract basic info]Waiting for validate project in basic info')
        await validateProject({...defaultConfig, path: tmpDir}, '[Extract basic info] ')

        console.log('[Extract basic info] Waiting for fillDeployConfig')
