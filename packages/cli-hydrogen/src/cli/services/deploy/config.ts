...
export const validateProject = async (config: DeployConfig, log: (value: string) => void = console.log) => {
  log(`waiting for ensurePresentOrAbort`)
  await git.ensurePresentOrAbort()
  try {
    log(`waiting for ensure inside git directory`)
    ...
    log(`waiting for initialising git`)
    await initializeGit(config, log)
  }
}

export const initializeGit = async (config: DeployConfig, log: (value: string) => void = console.log) => {
  if (!config.assumeYes) {
    log(`config not assume yes, running git init`)
    const shouldGitInit = await gitInit()
    if (!shouldGitInit) throw new error.AbortSilent()
  }

  log(`waiting for initialise repostory`)
  await git.initializeRepository(config.path)
  git.createGitIgnore(config.path, MINIMAL_GIT_IGNORE)
  log(`waiting for add all`)
  await git.addAll(config.path)
  log(`waiting for commit`)
