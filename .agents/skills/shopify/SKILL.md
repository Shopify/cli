...
- `--api-name` is an optional narrowing hint, not a required parameter. The valid set of API names, defined by shopify.dev, is: `admin`, `admin-extensions`, `checkout-ui-extensions`, `customer-account-ui-extensions`, `pos-ui-extensions`, `app-home`, `storefront`, `partner`, `customer`, `payments-apps`, `hydrogen`, `liquid`, `storefront-web-components`, and `functions` (`shopify doc search --help` shows only a few of these as examples). The CLI does not validate the flag: **unrecognized values are silently ignored** — unlike `--api-version`, where an invalid value errors and lists the valid ones. So never invent an API name (there is no `polaris`, for example — App Home is `app-home`): pass `--api-name` only with a value from the set above, and omit it otherwise. Searching across ALL APIs is the supported default and the generic fallback.
...
- `--api-name` is optional and unvalidated: shopify.dev silently ignores a value it doesn't recognize, so a wrong name quietly returns unscoped results instead of erroring — you won't be warned. Only pass a value from the enumerated set in step 1; when unsure, omit it and search all APIs. (`--api-version` is stricter — an invalid version errors and lists the valid ones.)
